...
指定した秒数を過ぎると worker は終了処理へ移り、新規接続の受付を
停止します。処理中の要求は可能な限り完了させます。既に受け付けて
HTTP 接続が ready になった新規接続についても、最初の1要求と
その応答を既存の C<shutdown_timeout> 内で完了させます。このように
最初の要求を待つ接続では、2要求目は処理しません。再利用待ちの
接続は従来どおり閉じます。

この動作には L<Web::Transport::GenericServerConnection> の
C<wait_for_first_request> 対応版が必要です。通信ライブラリだけ、
または Sarze だけを更新しても、この組合せの修正は完成しません。
worker の寿命や終了期限を延長する変更ではありません。
